...
function Menu(caller, options){
	...
	// ----- multi-level menu defaults -----
		...
		nextMenuLink: 'fa-caret-right', // class to style the link (specifically, a span within the link) used in the multi-level menu to show the next level
		topLinkText: 'All',
		nextCrumbLink: 'ui-icon-carat-1-e',
		functionChooseItem: null
	}, options);
...
if (options.flyOutOnState) {
	...
	this.chooseItem = function(item){
		...
		options.functionChooseItem(item);
	};
}
Menu.prototype.flyout = function(container, options) {
	...
	container.addClass('fg-menu-flyout').find('li:has(ul)').each(function(){
		...
		allSubLists.css({ left: linkWidth + 7, width: linkWidth }).hide();
			
		$(this).find('a:eq(0)').addClass('fg-menu-indicator').html('<span>' + $(this).find('a:eq(0)').text() + '</span><span class="fa '+options.nextMenuLink+'" style="float: right; padding-top: 5px;"></span>').hover(
			function(){
                ...
                if (!fitHorizontal(subList, $(this).offset().left + 107)) {
	...
	topList.find('a').each(function(){
		...
		if ($(this).next().is('ul')) {
			$(this)
				...
				.each(function(){ $(this).html('<span>' + $(this).text() + '</span><span class="fa '+options.nextMenuLink+'" style="float: right; padding-top: 5px;"></span>'); })
