formManager = (function () {
    ...
    let setup = function () {
        ...
        $('#thongTinCT').on('click', function () {
            ...
            let thoiHan = parseInt($('input[name="thoiHan"]').val());
            window.location = url + '/worklife/qlctgt/user/danhsachhs/xulyhoso/cpcaitaoviahe.do?idHoSo=' + idHoSo;
        });
        ...
        $('#diaDiem_DiaChi,#nhieuHuyen').on('change', function () {
            /*$('#nhieuHuyen option:selected').each(function(v,k){console.log($(k).text())});
            for(i=0;i<(lst= $('#nhieuHuyen option:selected'),lst.length);i++ ){console.log(lst[i])}*/
            let nhieuHuyen =[];
            $('#nhieuHuyen option:selected').each(function(v,k){
                nhieuHuyen += $(k).text()+ ', ';
            });
            nhieuHuyen = nhieuHuyen.slice(0,nhieuHuyen.length-2)+'';
            let val = '';
            $('#noiDung_CapPhep').val(val);
            let diaDiem_DiaChi = $('#diaDiem_DiaChi').val();
            $('#noiDung_CapPhep').val('Cải tạo vỉa hè trong phạm vi đất của đường bộ '+diaDiem_DiaChi + ' thuộc huyện: ' + nhieuHuyen)
        });
    };
    ...
    let btnSaveTTCT = function () {
        ...
        if (form.valid()) {
            ...
            form.attr("action", url + "/worklife/qlctgt/user/xulyhoso/hscpcaitaoviahe/saveForm.do?" + $("meta[name='_csrf_parameter']").attr('content') + '=' + $("meta[name='_csrf']").attr('content'));
    ...
    let initHuyendataG = function() {
        let idHoSo = parseInt($('input[name="idHoSo"]').val());
        $.ajax({
            url: url + '/worklife/qlctgt/user/xulyhoso/cpcaitaoviahe/selectnhieuhuyen.do',
            type: 'post',
            data: {idHoSo: idHoSo},
            success: function (data) {
                if (data !== null&& data!==""){
                    if (data.listIdHuyen !== null || data.listIdHuyen !== ""||data.listIdHuyen !== undefined){
    ...
    let loadDefaultGP = function(data) {
       /* let loaiHuyen = '';
        ...
        let nhieuHuyen= '' + (data.nhieuHuyenVO===null?'':data.nhieuHuyenVO.bphoneHuyen);
        let tieuDe =
            '\t' + 'Công trình: ' + data.congTrinhVO.tenCongTrinh + '\n'
            + '\t' + 'Địa điểm: ' + data.congTrinhVO.diaDiem_DiaChi + '' + nhieuHuyen;
        $('#tieuDe').val(tieuDe);
        let canCu = data.dmCanCuPhapLyVO.noiDung;
        $('#canCu').val(canCu);
        let phamViCP = '';
        ...
        let noiDungGP =
             '\t' + 'Xét Hồ sơ đề nghị cấp phép thi công ' + data.congTrinhVO.noiDung_CapPhep + ' kèm theo Đơn số ' + data.congTrinhVO.donXinCP + ' của ' + data.congTrinhVO.tenDoanhNghiep + '.' + '\n'
            + '\t' + 'Sở Giao thông vận tải Khánh Hòa cấp phép thi công các nội dung như sau: '  + '\n'
