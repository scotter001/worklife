$(document).ready(function() {
	...
	dataTable.initDataTable('tableRoleList', url + 'worklife/manager/permission/getListPermission.do');
	dataTable.initPageHolder('tableRoleList', 'pageNumber');
		            	...
		            	try {
			            	if (tableSetting != undefined) {
			                	...
			                	if (setting[0][0] != undefined) {
				                	switch(setting[0][0]) {
				                		case 1: columnName = "permission_name"; break;
				                		case 2: columnName = "permission_pattern"; break;
				                		case 3: columnName = "list_permission_name"; break;
				                		case 4: columnName = "list_permission_no"; break;
				                		case 5: columnName = "permission_description"; break;
				                	}
				                	if (columnName != "") {
					            		d.column = columnName;
		            ...
		            'dataSrc': function(json) {
		    			if (json != undefined && json != null && json.data != undefined && json.data != null) {
		    				for ( var i = 0, ien = json.data.length; i < ien; i++) {
		    					json.data[i][2] = '<a href="' + url + 'worklife/manager/permission/modify/modifyPermission.do?permissionId=' + json.data[i][0] + '" >' + json.data[i][2] + '</a>';
